...
#
            ...
            if not isUpgrade:
              ...
              logger.info(msg)
              print(msg)
              conn, rollback_ok = run_rollback(conn, device, vendor, device_name, rollback_image, host, logger,accepted_vendors)
              if not rollback_ok:
...
#  rollback = Rollback(decice, accepted_vendors, rollback_image)
  ...
  precheck = PreCheck(device, accepted_vendors)
  try:
    print(f"===============conn,inside_run_rollback==============={conn}")
    if vendor == "juniper":
      ...
      for details in reversed_list:
        ...
        print(f"===============conn,inside_run_rollback_2==============={conn}")
...
#                 f'{host}: Connected successfully')
        ...
        if not upgrade_success:
          ...
          logger.info(msg)
          print(msg)
          print(f"===============conn,run_rollback==============={conn}")
