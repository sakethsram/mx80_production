...
def run_rollback(conn, dev, device_key, rollback_image, logger):
    ...
    try:
        ...
        #rolled bak success or rolled bak failure (binary failure will exit the device and stop the device )
        device_results[device_key]["upgrade"]["status"] = "r"
        return conn, True
...
def run_upgrade(dev: dict, device_key: str, logger):
    ...
    try:
        # ── STEP 1: Connect ───────────────────────────────────────────────────
        try:
            #conn should be sent from the main - once and for all -not form each phase - each connection in upgrade is okay but not intitially
            conn = connect(device_key, dev, logger)
            if not conn:
                raise ConnectionError("connect() returned None")
